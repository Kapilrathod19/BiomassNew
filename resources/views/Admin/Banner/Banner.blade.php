        ...
        <div class="row g-4">
            <div class="col-sm-12 col-xl-12">
                <div class="bg-light rounded h-100 p-4 d-flex justify-content-between align-items-center">
                    ...
                    <a class="btn btn-primary rounded-pill" href="{{ route('admin.add.banner') }}" role="button">Add
                        Banner</a>
                </div>
            </div>
        </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <img src="{{ asset('banners/' . $banner->image) }}"
                                            alt="{{ $banner->name ?? 'Banner' }}" width="100px" height="50px"
                                            data-bs-toggle="modal" data-bs-target="#imageModal"
                                            data-image="{{ asset('banners/' . $banner->image) }}">
                                    </td>
                                    <td>{{ $banner->name ?? 'N/A' }}</td>
                                    <td>{{ $banner->updated_at->format('Y-m-d H:i:s') }}</td>
                                    <td>
                                        <a class="btn btn-primary" href="{{ route('admin.edit.banner', $banner->id) }}"
                                            role="button"><i class="fas fa-edit"></i></a>
                                        <a class="btn btn-danger" href="{{ route('admin.delete.banner', $banner->id) }}"
